...
EXPERT_SYSTEM_PROMPT = """You are a GPU pricing expert. Your job is to analyze current GPU prices and determine if they are a bargain.

IMPORTANT: You MUST perform a web search to find CURRENT prices. Do NOT guess.

Provide a detailed "computational justification" that includes:
...
5. Final "Bargain Score" from 0-100
...
async def query_expert(browser: Browser, expert_class: type, gpu_model: str) -> dict[str, Any]:
    ...
    try:
        ...
        message = f"{EXPERT_SYSTEM_PROMPT}\n\nPlease analyze the current price and value of {gpu_model}. What is the current price and is it a good deal? Provide your Bargain Score."
...
async def main():
    """Run oracle for each GPU model"""
    for model in GPU_MODELS:
        await run_oracle(model)
